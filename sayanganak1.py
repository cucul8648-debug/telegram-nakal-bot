...
async def receive_content(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Header image & konfigurasi channel
...
async def auto_accept_join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Terima otomatis permintaan join ke grup/channel"""
    chat = update.chat_join_request.chat
    user = update.chat_join_request.from_user
    await context.bot.approve_chat_join_request(chat.id, user.id)
    logger.info(f"✅ {user.first_name} diterima otomatis ke {chat.title}")

    try:
        await context.bot.send_message(
            chat_id=user.id,
            text=f"Halo {user.first_name}! 👋\nKamu sudah diterima ke grup {chat.title} 💫"
        )
    except Exception as e:
        logger.warning(f"Gagal kirim pesan sambutan: {e}")
...
bot_app.add_handler(ChatJoinRequestHandler(auto_accept_join))
...
def home():
    return "Bot aktif dan siap kerja 💫", 200

# ================== JALANKAN APP ==================
if __name__ == "__main__":
    target_url = f"{RENDER_URL}/{TOKEN}"
    try:
        r = requests.get(f"https://api.telegram.org/bot{TOKEN}/setWebhook?url={target_url}")
        if r.status_code == 200:
            logger.info(f"Webhook diset ke: {target_url}")
        else:
