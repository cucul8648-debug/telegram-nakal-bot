# filename: main_final_render_autoset.py
...
# ================== AUTO WEBHOOK SAAT STARTUP ==================
@flask_app.before_serving
async def before_serving():
    """Otomatis set webhook sebelum Flask menerima request pertama"""
    try:
        current = await bot_app.bot.get_webhook_info()
        target_url = f"{RENDER_URL}/{TOKEN}"
        if current.url != target_url:
            await bot_app.bot.set_webhook(target_url)
            logger.info(f"Webhook otomatis diset: {target_url}")
        else:
            logger.info(f"Webhook sudah aktif: {current.url}")
    except Exception as e:
