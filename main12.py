# main_polling_final.py
# requirements: python-telegram-bot==20.3 python-dotenv==1.0.1
...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.chat.type != "private":
        return
    await update.message.reply_text("👋 Selamat datang di Nabrutt 🤖\n\nPilih jenis kelaminmu terlebih dahulu:", reply_markup=gender_keyboard())
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data=="repost_last":
        user_data.clear()
        await query.message.edit_text("👋 Selamat datang di Nabrutt 🤖\n\nPilih jenis kelaminmu terlebih dahulu:", reply_markup=gender_keyboard())
        return
